...
<div class="container">
    <div class="row d-flex align-items-center ">
        ...
        <div class="col-9">
            ...
            <h6 style="margin-top: -8px;">TAHUN PELAJARAN 2024/2025</h6>
            </div>
        </div>
    </div>
...
<div class="row d-flex justify-content-end">
    <div class="col-7"><p class="fw-bold text-start" style="font-size: 14px;">Kode Referal : @if ($pd->referral) {{$pd->referral}}@else - @endif</p></div>
    <div class="col-5"><p class="fw-bold text-end" style="font-size: 14px;">Kode Pendaftaran : @if ($pd->id_users) {{$pd->id_users}}@else error @endif</p></div>
</div>
    ...
    <tbody>
        ...
        <tr>
          ...
          <td>@if ($pd->tempat_lahir_ayah){{$pd->tempat_lahir_ayah}}@else - @endif
            @if ($pd->tgl_lahir_ayah) {{Carbon\Carbon::parse($pd->tgl_lahir_ayah)->format('d M Y')}} @else - @endif  
          </td>

          <td>@if ($pd->tempat_lahir_ibu){{$pd->tempat_lahir_ibu}}@else - @endif
              @if ($pd->tgl_lahir_ibu) {{Carbon\Carbon::parse($pd->tgl_lahir_ibu)->format('d M Y')}} @else - @endif
            </td>

          <td>@if ($pd->tempat_lahir_wali){{$pd->tempat_lahir_wali}}@else - @endif
                @if ($pd->tgl_lahir_wali) {{Carbon\Carbon::parse($pd->tgl_lahir_wali)->format('d M Y')}} @else - @endif
              </td>
        </tr>
        <tr>
